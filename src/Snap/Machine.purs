...
mapO :: ∀ a b i s. (a -> b) -> Machine s i a -> Machine s i b
mapO f = (Step.mapO f <<< _)

mapI :: ∀ a b o s. (a -> b) -> Machine s b o -> Machine s a o
mapI f = (Step.mapI f <<< _)

mapS :: ∀ s s' i o. (s' -> s) -> (s -> s') -> Machine s i o -> Machine s' i o
mapS f g m = Step.mapS g <<< m <<< f

lstrength :: ∀ x i o s. Machine s i o -> Machine (s /\ x) i o
lstrength m (s /\ x) = case m s of
  o /\ t -> o /\ (_ /\ x) <<< t

rstrength :: ∀ x i o s. Machine s i o -> Machine (x /\ s) i o
rstrength m (x /\ s) = case m s of
  o /\ t -> o /\ (x /\ _) <<< t

ultraleft :: ∀ x y i o s. Machine s i o -> Machine s (i \/ x) (o \/ y)
ultraleft m s = bimap Left (\f -> either f (const s)) $ m s

ultraright :: ∀ x y i o s. Machine s i o -> Machine s (x \/ i) (y \/ o)
ultraright m s = bimap Right (\f -> either (const s) f) $ m s

divergent :: ∀ s i1 i2 o1 o2. Machine s i1 o1 \/ Machine s i2 o2 -> Machine s (i1 \/ i2) (o1 \/ o2)
divergent = either ultraleft ultraright

independent :: ∀ s1 s2 i1 i2 o1 o2. Machine s1 i1 o1 /\ Machine s2 i2 o2 -> Machine (s1 /\ s2) (i1 /\ i2) (o1 /\ o2)
