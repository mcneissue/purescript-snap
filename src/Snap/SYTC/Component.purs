...
dimap f g cmp u s = cmp (u P.<<< g) (f s)
...
first cmp u s = let c = snd s in cmp (u P.<<< flip Tuple c) (fst s)
...
zip c c' set (Tuple s s') = c (set P.<<< flip Tuple P.mempty) s <> c' (set P.<<< Tuple P.mempty) s'
...
zipMono c c' set v@(Tuple s s') = zip' set' v
  ...
  set' = set P.<<< bimap ((_ $ s) P.<<< runEndo) ((_ $ s') P.<<< runEndo)

contraHoist :: forall v s u m n. (n Unit -> m Unit) -> Cmp m v s u -> Cmp n v s u
contraHoist f cmp set s = cmp (f P.<<< set) s
...
bind c f = curry $ (uncurry c) >>= (f P.>>> uncurry)
...
handle f c set s = c (flip f s P.>>> set) s
